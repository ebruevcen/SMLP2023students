...
```{julia}
#| label: data
data1 = DataFrame(Arrow.Table("data/Pashkova_SpeechRate.arrow"));
describe(data1)
levels(data1.Group)
```
...
m1 = let
    ...
    fit(MixedModel, form, data1; contrasts=contr1);
  end
```
...
m2 = let
    form = @formula(rate ~ 1 + Group + (1 | Spkr));
    fit(MixedModel, form, data1; contrasts=contrasts2);
  end
```
...
```{julia}
##Intercept here is the Grand Mean, you can specify what to compare what.
m3 = let levels = ["English_MS", "German_HS", "Greek_HS", "Russian_HS", "Turkish_HS"]
  ...
  form = @formula(rate ~ 1 + Group + (1 | Spkr));
  fit(MixedModel, form, data1; contrasts=contrasts3)
end
```
